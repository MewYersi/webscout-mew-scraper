...
PESOS_JSON = {
    "DC": {
        ...
        "estadisticas": {
            ...
            "Passing": {
                ...
                "PrgDist": 0.20,    # Avanzar metros con el balón
                "Prog": 0.20,       # Pases que rompen líneas
                "LongCmp": 0.20,    # Cambios de frente / Balones largos
                "Cmp%Long": 0.15    # Precisión en ese pase largo
            },
            "Possession": {
                "TouchesDef3rd": 0.45, # Involucramiento en la base de la jugada
                "TouchesMid3rd": 0.25, # Capacidad de empujar al equipo arriba
                "Dispossesed": 0.30    # Seguridad ante la presión (Inversa)
            }
        }
    },
    "LB_DEF": {
        ...
        "estadisticas": {
            "Defensa": {
                "TklW": 0.25,       # Gana el 1 vs 1 en banda
                "Int": 0.25,        # Corta líneas de pase
                "Recov": 0.20,      # Regresa rápido
                "Clr": 0.15,        # Cierra el segundo palo
                "TklDef3rd": 0.15   # Fuerte cerca de su área
            },
            "Passing": {
                "Cmp%Total": 0.35,  # Pase seguro en salida
                "PrgDist": 0.25,    # Saca al equipo del fondo
                "Prog": 0.25,       # Pases verticales
                "IntoLast3rd": 0.15 # Conecta con los mediocampistas/extremos
            },
            "Possession": {
                "TouchesDef3rd": 0.40,
                "TouchesMid3rd": 0.30,
                "Dispossesed": 0.30 # Inversa: No perderla presionado
            },
            "Aerial": {
                "AerialWon%": 0.70, # Importante para cerrar centros al segundo palo
                "AerialWon": 0.30
            }
        }
    },
    "LB_OFF": {
        ...
        "estadisticas": {
            "Passing": {
                "xA": 0.30,         # Peligro real generado
                "CrsPA": 0.25,      # Centros exitosos al área
                "KP": 0.20,         # Pases clave
                "Prog": 0.15,       # Pases progresivos
                "IntoLast3rd": 0.10
            },
            "Possession": {
                "TouchesAtt3rd": 0.40, # Vive en campo rival
                "PrgDist": 0.30,       # Conducciones largas
                "DribSucc": 0.20,      # Rompe líneas en el 1v1
                "Dispossesed": 0.10
            },
            "GAS": {
                "SCA": 0.60,        # Acciones de creación de tiro
                "SCAPassLive": 0.40 # Peligro con balón en movimiento
            },
            "Defensa": {
                "Recov": 0.30,      # Qué tan rápido baja tras atacar
                "TklW": 0.30,
                "Int": 0.25,
                "BallRec": 0.15
            }
    ...
    "MC_EST": {
        "categoria_pesos": { "Passing": 0.28, "Possession": 0.22, "Defensa": 0.22, "GAS": 0.15, "Shooting": 0.08, "Aerial": 0.05 },
        "estadisticas": {
            "Passing": { "Prog": 0.15, "IntoLast3rd": 0.15, "PrgDist": 0.15, "xA": 0.15, "Cmp%Total": 0.10, "KP": 0.10, "PPA": 0.10, "Ast": 0.10 },
            "Defensa": { "Recov": 0.25, "TklMid3rd": 0.20, "TklW": 0.20, "Int": 0.15, "BallRec": 0.10, "BallRecProg": 0.10 },
            "Possession": { "TouchesMid3rd": 0.20, "TouchesAtt3rd": 0.15, "TouchesDef3rd": 0.15, "PrgDist": 0.15, "TouchesAttPen": 0.10, "DribSucc": 0.10, "Dispossesed": 0.10, "BallRecProg": 0.05 },
            "GAS": { "SCA": 0.35, "SCAPassLive": 0.30, "GCA": 0.20, "SCADef": 0.10, "SCADrib": 0.05 },
            "Aerial": { "AerialWon%": 0.60, "AerialWon": 0.40 },
            "Shooting": { "Gls": 0.30, "npxG": 0.25, "G-xG": 0.20, "xG": 0.15, "SoT%": 0.10 }
        }
    },
    # === LA HIPER-ESPECIALIZACIÓN DE LOS ATACANTES ===
    "AM": {
        "categoria_pesos": { "Passing": 0.45, "GAS": 0.40, "Shooting": 0.10, "Possession": 0.05 },
        "estadisticas": {
            "Passing": { "xAG": 0.20, "xA": 0.20, "KP": 0.20, "PPA": 0.15, "Prog": 0.15, "IntoLast3rd": 0.10 },
            "GAS": { "SCA": 0.30, "SCAPassLive": 0.30, "GCA": 0.20, "SCAPassDead": 0.10, "SCADrib": 0.10 },
            "Shooting": { "Gls": 0.30, "npxG": 0.30, "np:G-xG": 0.20, "npxG/Sh": 0.20 },
            "Possession": { "TouchesAtt3rd": 0.50, "TouchesAttPen": 0.50 }
        }
    },
    "FW_WG": {
        "categoria_pesos": { "Possession": 0.35, "GAS": 0.25, "Passing": 0.20, "Shooting": 0.20 },
        "estadisticas": {
            "Possession": { "DribSucc": 0.40, "PrgDist": 0.40, "TouchesAtt3rd": 0.20 },
            "GAS": { "SCADrib": 0.60, "GCA": 0.30, "SCAFld": 0.10 },
            "Passing": { "CrsPA": 0.50, "xAG": 0.30, "IntoLast3rd": 0.20 },
            "Shooting": { "Gls": 0.50, "npxG": 0.50 }
        }
    },
    "FW_ST": {
        "categoria_pesos": { "Shooting": 0.60, "GAS": 0.25, "Aerial": 0.10, "Passing": 0.05 },
        "estadisticas": {
            "Shooting": { "Gls": 0.40, "npxG": 0.30, "np:G-xG": 0.20, "npxG/Sh": 0.10 },
            "GAS": { "GCA": 0.50, "SCA": 0.50 },
            "Aerial": { "AerialWon": 0.50, "AerialWon%": 0.50 },
            "Passing": { "xA": 0.50, "Ast": 0.50 }
        }
    },
    "FW_SS": {
        "categoria_pesos": { "Shooting": 0.30, "GAS": 0.30, "Passing": 0.30, "Possession": 0.10 },
        "estadisticas": {
            "Shooting": { "Gls": 0.40, "npxG": 0.40, "npxG/Sh": 0.20 },
            "GAS": { "SCA": 0.50, "SCAPassLive": 0.50 },
            "Passing": { "PPA": 0.40, "KP": 0.30, "xA": 0.30 },
            "Possession": { "TouchesAttPen": 0.70, "DribSucc": 0.30 }
        }
    }
}
...
def calcular_valor_deportivo(row, col_rtg, nombre_rol):
    ...
    # Curva Exponencial Base (Se mantiene: premia la élite)
    precio_base = ((rtg / 10) ** 6) * 200  

    # 1. EDAD: Aplanada. Ya no premiamos el potencial de venta. 
    # Solo aplicamos un micro-ajuste de fiabilidad física extrema.
    factor_edad = 1.0
    if edad >= 34: 
        factor_edad = 0.90 # Pequeño riesgo de declive físico/lesiones a esa edad
    
    # 2. MINUTOS: Se mantiene. Quien no juega, no aporta valor deportivo real.
    pct_minutos = minutos / 3420
    factor_minutos = 0.6 + (pct_minutos * 0.4)
    if minutos < 1000: 
        factor_minutos = 0.5

    # 3. LIGA: Se mantiene. La dificultad del torneo es clave.
    factor_liga = 1.0
    ...
    # 4. POSICIÓN: Democratizada. El fútbol es un sistema de 11.
    prefijo = nombre_rol.split('_')[0]
    if prefijo in ['FW', 'AM']:
        factor_pos = 1.10 # Ligerísimo bono por la extrema dificultad de generar gol
    elif prefijo == 'MC':
        factor_pos = 1.0
    else: # Defensas
        factor_pos = 0.98 # Casi a la par, defender bien vale puntos
