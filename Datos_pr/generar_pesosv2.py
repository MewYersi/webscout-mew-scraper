...
def calcular_rating_rol(df, rol_analisis):
    ...
    for col in cols_existentes:
        peso = pesos[col]
        nombre_contrib = f"Contrib_{col}"
        df_calc[nombre_contrib] = df_calc[f"{col}_Norm"] * peso
        df_calc[col_rating] += df_calc[nombre_contrib]
    
    df_calc[col_rating] = df_calc[col_rating] * 100
    
    cols_finales = ['Player', 'Squad', 'Pos', 'PosAdj', 'Min', col_rating] + [c for c in df_calc.columns if "Contrib_" in c]
    return df_calc[cols_finales].sort_values(col_rating, ascending=False)

# ==========================================
# 3. EJECUCIÓN CON FILTRO INTELIGENTE
# ==========================================
...
if os.path.exists(archivo):
    ...
    for rol in PESOS_JSON.keys():
        ...
        for i, row in top_3.iterrows():
            contribs = {k:v for k,v in row.items() if "Contrib_" in str(k)}
            top_stats = sorted(contribs.items(), key=lambda x: x[1], reverse=True)[:2]
            stats_txt = ", ".join([f"{k.replace('Contrib_','')}" for k, v in top_stats])
            print(f"   {i+1}. {row['Player']} ({row[f'RTG_{rol}']:.1f}) -> Clave: {stats_txt}")
            
        df_resultado.to_excel(writer, sheet_name=rol, index=False)
    
    writer.close()
    print("\n[OK] Archivo 'Resultados_Tesis_Filtrado.xlsx' generado con éxito.")
